{ ... }: {
  # Overlay/mesh networking aspect: yggdrasil + i2pd + Reticulum (rnsd/lxmd) +
  # microsocks. Opt-in (hosts add `config.flake.modules.nixos.mesh`); NOT part
  # of the universal `cli` base, so resource-constrained `embedded` boards don't
  # run it. Extracted from the old monolithic `default`.
  flake.modules.nixos.mesh =
{
  config,
  lib,
  pkgs,
  ...
  # Mesh CLI tooling (moved out of base.nix systemPackages).
  ...
  # SOCKS5 proxy (was socks-proxy.nix).
  services.microsocks = {
    enable = true;
    ip = "0.0.0.0";
    port = 1080;
